...
(function (global) {
  ...
  const T = {
    sunset: {
      ...
      // 2026-09-22: 採点そのものは実写から当てたモデルへ移した（AFTERGLOW_MODEL）。
      // 特許は窓の取り方・太陽方位側を見る距離・「光を受ける面」の考え方として残っている。
      source: "環境省の定点カメラの実写1,298事象に当てたモデル。窓と太陽方位側の距離は SunsetWx特許 US10459119 に基づく",
    },
    starry: {
    ...
    seaOfClouds: {
      ...
      //
      // 正本: [[ドメイン/開発/絶景日和/雲海/02_検証結果]] 検証14。
    ...
    // 75→25→5→0 と3回下げた。5 にしたのは既存の「出ない」ケース（2点・5点）へ
    // 揃えただけで、その2点・5点自体が妥当かを問い直していなかった。
    // **出ないと分かっているなら 0。** 0 は欠測ではない（欠測は unavailable で返す）。
    // 日没間際の淡い虹は、この上限ではなく directLo/directHi の傾斜で扱う。
    noSunDirect: 3, noSunCeiling: 0,
  ...
  // ---------------------------------------------------------------- 朝夕焼けの学習モデル
  // 環境省の定点カメラの実写と突き合わせて当てた加法モデル。**規則ではなく実写が出した形。**
  //
  // 6つの入力それぞれを折れ線（25/50/75% に節。降水は 0.1/0.5/2.0mm）に展開し、
  // log1p(写真の色づき) を当てた Ridge（alpha=10）。学習は2023-2024年・加太と五色台の夕焼け。
  // **1入力＝1つの形**なので、「この点数のわけ」を要因ごとに出せる形は保てる。
  //
  // 学習外での一致（地点内の順位・写真指標または人手採点との Spearman）:
  //   人手採点164枚（2018年・4カメラ・朝夕） 現行0.395 → 0.635
  //   五色台 2025-2026年 335事象            現行0.314 → 0.575
  //   2024年 426事象（加太・五色台）          現行0.446 → 0.647
  //
  // calibX/Y は 0〜100 への対応表。同じ1,298事象で**現行規則が出す点数の分布**へ順位で
  // 合わせてある。順位はモデルのもの、目盛りは現行のもの。絶景の出る頻度は 4.2% → 4.0%。
  // 同じ予測値の塊（雲ひとつない空は6入力すべて0で完全に一致する）には塊の真ん中の順位を配る。
  // 塊の最後の順位を配ると、快晴の日が軒並み100点になる。
  //
  // 出典と検証: Vault ドメイン/開発/絶景日和/朝焼け・夕焼け/02_検証結果
  //             係数の作り直しは 検証データ/20260920_複数地点設計資料/研究_本番モデル書き出し.py
  const AFTERGLOW_MODEL = {
    features: ["home_low", "home_mid", "home_high", "precip", "sun_low_low", "sun_high_high"],
    knots: {
      home_low: [25, 50, 75],
      home_mid: [25, 50, 75],
      home_high: [25, 50, 75],
      precip: [0.1, 0.5, 2.0],
      sun_low_low: [25, 50, 75],
      sun_high_high: [25, 50, 75],
    },
    // 学習が見た範囲。**外は外挿になるので、そこへは出さない。**
    range: {
      home_low: [0.0, 100.0],
      home_mid: [0.0, 100.0],
      home_high: [0.0, 100.0],
      precip: [0.0, 9.8],
      sun_low_low: [0.0, 100.0],
      sun_high_high: [0.0, 100.0],
    },
    // 増えて良くなることはない入力。学習の形は最後の節より先で反転していた
    // （雲ひとつない空に30mmの雨、という学習に無い組み合わせで点数が上がっていた）。
    // 実写が言えるのはここまで、という線を引く。
    neverHelps: ["precip"],
    // ふつうの日（各入力の学習時の中央値）。内訳はここからの差として出す。
    reference: {"home_low": 8.5, "home_mid": 15.5, "home_high": 28.5, "precip": 0.0, "sun_low_low": 9.5, "sun_high_high": 11.5},
    referenceY: 1.708616,
    y0: 1.2791372754865211,
    mu: [
      18.6852, 7.39688, 3.01628, 0.733379, 28.2388, 14.5299, 7.00678, 2.4152,
      43.114, 29.0814, 17.3073, 7.2863, 0.196879, 0.173948, 0.121031, 0.0442334,
      22.1696, 10.2639, 4.55427, 1.42809, 38.3501, 25.8046, 15.4878, 6.59159,
    ],
    sd: [
      24.4815, 17.1026, 9.51445, 3.56455, 32.1121, 24.2229, 15.0264, 6.56074,
      43.0702, 32.8104, 21.7061, 10.4723, 0.734203, 0.714475, 0.640137, 0.444472,
      28.2651, 20.5202, 12.2433, 5.16258, 42.8983, 32.4654, 21.2991, 10.2674,
    ],
    b: [
      -0.0829899, 0.0326299, -0.0294324, -0.000623947, -0.17511, -0.100606, -0.0781091, 0.0937572,
      -0.00824341, -0.0177417, 0.0279315, -0.175246, -0.125242, -0.0307284, 0.129073, 0.00917942,
      -0.232948, 0.143985, 0.00170314, -0.0404141, -0.0993289, -0.00849614, 0.0678239, -0.114175,
    ],
    calibX: [
      -0.556573, 0.283046, 0.302925, 0.32637, 0.337712, 0.371618, 0.376675, 0.383968,
      0.424983, 0.431244, 0.434953, 0.440123, 0.445955, 0.4477, 0.456708, 0.458126,
      0.507239, 0.512944, 0.527626, 0.52878, 0.543001, 0.556419, 0.586005, 0.598421,
      0.600371, 0.616669, 0.619126, 0.63545, 0.640093, 0.664654, 0.664937, 0.667698,
      0.672516, 0.686975, 0.698796, 0.703359, 0.730205, 0.74043, 0.913927, 0.922723,
      0.924394, 0.925349, 0.925449, 0.95256, 0.957805, 0.958609, 0.958656, 0.962356,
      0.972758, 0.976501, 0.991272, 1.00809, 1.0094, 1.0157, 1.01684, 1.01751,
      1.02277, 1.02712, 1.04569, 1.05496, 1.08317, 1.09069, 1.13916, 1.14919,
      1.17463, 1.20849, 1.22663, 1.23086, 1.24255, 1.34191, 1.35867, 1.40471,
      1.4149, 1.4579, 1.46238, 1.48044, 1.51233, 1.54208, 1.5489, 1.67236,
      1.72293, 1.77646, 1.81798, 1.82725, 1.8532, 1.85504, 1.89132, 1.90335,
      1.90742, 1.91042, 1.91667, 1.91686, 1.92133, 1.92598, 1.93007, 1.9301,
      1.93021, 1.9315, 1.93243, 1.93307, 1.93422,
    ],
    calibY: [
      0, 0, 1.562, 2.369, 3.361, 4.48, 5.2, 6.991,
      9.317, 10.667, 10.667, 11.667, 11.789, 13.105, 13.148, 13.877,
      15.562, 16.489, 17.421, 18.508, 18.604, 20, 20.526, 21.451,
      21.944, 22.778, 23.313, 23.771, 24.692, 25, 25.462, 25.938,
      25.942, 27.67, 28.121, 28.744, 29.087, 30, 30, 30.333,
      30.75, 31.479, 32.432, 33.063, 34.25, 35.042, 36.587, 37.532,
      37.75, 38.463, 39.375, 39.736, 40.4, 40.625, 41.634, 41.739,
      41.762, 42.596, 43.289, 44.826, 46.5, 47.778, 48.912, 49.911,
      51.028, 53.607, 54.515, 55.25, 56.062, 58.85, 60, 60.025,
      60.826, 62.333, 62.889, 63.139, 64.625, 65.311, 65.984, 70,
      70, 72.5, 73.75, 74.6, 75.643, 76.15, 77.951, 78,
      78.25, 78.971, 79.25, 79.789, 80.144, 81.381, 81.884, 82.532,
      82.799, 83.214, 84.347, 85.927, 90.937,
    ],
  };

  // 折れ線の基底。1入力＝[値, max(0,値-節1), max(0,値-節2), max(0,値-節3)]。
  const hinges = (v, knots) => [v, ...knots.map((k) => Math.max(0, v - k))];

  // 予測（log1p(写真の色づき)）と、入力ごとの寄与を返す。
  // 寄与は**ふつうの日からの差**。「基準からどれだけ良い/悪い日か」を要因ごとに言える。
  //
  // 学習の範囲外へは出さない。範囲の中でも、増えて良くなるはずのない入力（降水）は
  // **そこまでの最小値で止める**。節より先は標本が薄く、形が反転していた。
  function afterglowPredict(values) {
    const M = AFTERGLOW_MODEL;
    const per = {};
    let yRef = M.y0, col = 0;
    for (const f of M.features) {
      const ks = M.knots[f], [lo, hi] = M.range[f];
      const v = Curve.clamp(values[f], lo, hi);
      const ref = hinges(M.reference[f], ks);
      const at = (x) => {
        const h = hinges(x, ks);
        let c = 0;
        for (let i = 0; i < h.length; i++) {
          c += M.b[col + i] * ((h[i] - M.mu[col + i]) / M.sd[col + i] - (ref[i] - M.mu[col + i]) / M.sd[col + i]);
        }
        return c;
      };
      let c = at(v);
      if (M.neverHelps.includes(f)) {
        // 折れ線なので、最小は節のどれか、端のどちらか。そこだけ見れば足りる。
        for (const x of [lo, ...ks].filter((x) => x <= v)) c = Math.min(c, at(x));
      }
      per[f] = c;
      for (let i = 0; i < ref.length; i++, col++) yRef += M.b[col] * (ref[i] - M.mu[col]) / M.sd[col];
    }
    let y = yRef;
    for (const f of M.features) y += per[f];
    return { y, yRef, per };
  }

  // 予測を 0〜100 へ載せ替える。対応表の外側は端の値で止める。
  function afterglowCalibrate(y) {
    const x = AFTERGLOW_MODEL.calibX, v = AFTERGLOW_MODEL.calibY;
    if (y <= x[0]) return v[0];
    if (y >= x[x.length - 1]) return v[v.length - 1];
    let lo = 0, hi = x.length - 1;
    while (hi - lo > 1) { const mid = (lo + hi) >> 1; if (x[mid] <= y) lo = mid; else hi = mid; }
    const t = (y - x[lo]) / (x[hi] - x[lo]);
  ...
  function afterglowScorer(kind) {
    ...
    return {
      ...
      // --- 実写から当てたモデルでの採点 ---
      // 6入力が揃わなければ null を返し、呼び出し側が規則へ落ちる。
      modelScore(window, input, home) {
        const [ws, we] = window;
        ...
        const precip = input.home.max("precipitation", ws, we);
        if (sunwardLow === null || sunwardHigh === null || precip === null) return null;

        const values = { home_low: home.low, home_mid: home.mid, home_high: home.high,
          precip, sun_low_low: sunwardLow, sun_high_high: sunwardHigh };
        const { y, yRef, per } = afterglowPredict(values);
        const score = afterglowCalibrate(y), base = afterglowCalibrate(yRef);

        // 内訳は予測のままでは点数の単位にならない（予測は log1p(色づき)）。
        // **同じ倍率で換算して、合計が表示スコアとぴったり合うようにする。**
        // 倍率は「ふつうの日からこの日までの、対応表の平均の傾き」。
        const span = y - yRef;
        const k = Math.abs(span) > 1e-9 ? (score - base) / span
          : (afterglowCalibrate(yRef + 0.01) - base) / 0.01;
        // 説明は短く、向きは符号と揃える。**実写が言っていることだけを書く。**
        // 「上層の雲は面になる」と書いて減点を出すと、文と記号が食い違って読めなくなる。
        const label = {
          home_high: () => [`上層の雲 ${pct(home.high)}`, "実写では、上層の雲は多いほど染まりが弱い"],
          home_mid: () => [`中層の雲 ${pct(home.mid)}`, "中層の雲が多いと光が抜けません"],
          home_low: () => [`下層の雲 ${pct(home.low)}`, "低い雲は頭上をふさぎます"],
          precip: () => [`降水 ${f1(precip)}mm`, "雨が降っていると空は染まりません"],
          sun_low_low: () => [`${direction}の下層雲 ${pct(sunwardLow)}`, `約165km先。ここが厚いと${sunlight}が届きません`],
          sun_high_high: () => [`${direction}の上層雲 ${pct(sunwardHigh)}`, "約368km先の上層の雲"],
        };
        const factors = AFTERGLOW_MODEL.features.map((f) => {
          const [name, why] = label[f]();
          const c = k * per[f];
      ...
      score(window, input) {
        ...
        // **実写から当てたモデルで採点する。** 6入力が揃っていればこちら。
        // 揃わないとき（太陽方位側を取っていない・降水が来ない）は下の規則へ落ちる。
        // 規則のほうは残してある。落とし先が無いと、欠測のたびに採点そのものが消える。
        const modelled = this.modelScore(window, input, { high, mid, low });
  ...
  const rainbowScorer = {
    ...
    score(window, input) {
      ...
      if (best) return best;
      // 条件不成立も点数として返す（母数に残す）。unavailable にはしない。
      // 点数はすべて 0。どれも「出ない」であって、
      // 出ない理由の違いは点数ではなく行の文言が持つ。
      // 寄与を付けると記号が ◎ になり、雨が無いのに好条件のように見えてしまう。
      if (sawRainHighSun) {
        return buildScore(0, [factor("雨は太陽が高い時間帯だけ", 0,
          "太陽が高すぎると、虹は地平線の下に隠れます")]);
      }
      if (sawRain) {
        return buildScore(0, [factor("雨は夜のあいだだけ", 0, "太陽が出ていなければ虹は出ません")]);
      }
      return buildScore(0, [factor("雨の予報がない", 0, "雨粒がなければ虹は出ません")]);
    },
  };
  ...
  //   朝焼け・夕焼け … 同じ判定（実写に当てたモデル）で、見え方も対になる
  //   星空           … 夜
  //   虹             … 昼
  ...
  const PHENOMENA = {
    ...
    sunrise: { name: "朝焼け", icon: "🌅", order: 0, record: "quality",
      ...
      says: ["朝の空が一面染まるかもしれません",
             ...
             "淡く色づく程度になりそうです",
             "色づきは弱そうです"] },
    // 富士山と月は sorami-fuji.js / sorami-moon.js が採点する（core は採点しない）。
    // **表に並べるための名前と語彙はここに置く。** 器を分けると、行の見た目が揃わない。
    //
    // `highlight: false` は「次の見どころ」に出さない印。
    // 他の現象は起きるか起きないかで、起きれば出かける理由になる。
    ...
    moon: { name: "月", icon: "🌙", order: 3, record: "quality", highlight: false,
      ...
      says: ["澄んだ空にくっきり浮かびそうです",
             ...
             "雲や霞ごしになりそうです",
             "見えにくそうです"] },
    // 笠雲も「次の見どころ」には出さない（`highlight: false`、2026-09-17 ユーザー判断）。
    // 月・富士山とは理由が違う。笠雲は材料が揃った日にだけ出る現象だが、**高い点でも実際に
    // 見えるのは 85点以上で約8%・65点以上で約6%**（日単位、2026-09-17）。見どころの先頭に
    // 出すと「行けば見られる」と誘う形になり、十数回に1回しか当たらない。表の行には残す。
    ...
    capCloud: { name: "笠雲", longName: "富士山の笠雲", icon: "🗻", order: 5.5, record: "occurrence", highlight: false,
      ...
      // **「かかる」と言わない。** 笠雲が見える日は50日に1日ほどで、85点以上でも実際に
      // 見えるのは約8%、65点以上で約6%（日単位、2026-09-17）。言えるのは「出やすさ」まで
      says: ["笠雲がとても出やすい条件です（ふだんの4倍ほど）",
